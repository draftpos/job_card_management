{
    'name': 'Job Card Management',
    'version': '19.0.2.0.2',
    'category': 'Services',
    'author': 'Odoo Development Team',
    'license': 'LGPL-3',
    'depends': ['sale', 'sale_management', 'purchase', 'purchase_requisition', 'stock', 'account', 'hr', 'hr_expense', 'mail'],
    'data': [
        'static/security/job_card_groups.xml',
        'static/security/ir.model.access.csv',
        'data/vehicle_catalog_seed.xml',
        'data/vehicle_records_seed.xml',
        'data/taxes_data.xml',
        'static/security/job_card_dashboard_access.xml',
        'reports/job_card_report.xml',
        'reports/job_cost_report.xml',
        'reports/account_move_report_views.xml',
        'reports/purchase_order_report_views.xml',
        'reports/picking_report_views.xml',
        'data/inventory_issue_sequence.xml',
        'wizard/print_invoice_wizard_views.xml',
        'wizard/grv_remaining_po_wizard_views.xml',
        'wizard/excess_warning_wizard_views.xml',
        'wizard/purchase_order_wizards_views.xml',
        'views/menu_root_views.xml',
        'views/technician_views.xml',
        'views/customer_views.xml',
        'views/vehicle_views.xml',
        'views/estimate_views.xml',
        'views/job_card_views.xml',
        'views/job_cost_views.xml',
        'views/inventory_issue_views.xml',
        'views/job_card_portal_templates.xml', 
        'views/procurement_views.xml',
        'views/procurement_reject_wizard_views.xml',
        'views/consumable_issue_views.xml',
        'views/consumable_issue_success_wizard_views.xml',
        'views/purchase_views.xml',
        'views/stock_picking_views.xml',
        'views/job_card_profitability_views.xml',
        'views/menu_views.xml',
        'views/job_card_dashboard_views.xml',
        'views/res_config_settings_views.xml',
        'views/estimate_portal_templates.xml',
    ],
    'assets': {
        'web.assets_backend': [
            'job_card_management/static/src/scss/job_card_dashboard.scss',
            'job_card_management/static/src/js/job_card_dashboard.js',
            'job_card_management/static/src/xml/job_card_dashboard.xml',
            'job_card_management/static/src/scss/report_preview.scss',
            'job_card_management/static/src/js/consumable_redirect_countdown.js',
        ],
        'web.assets_frontend_lazy': [
            'job_card_management/static/src/js/sale_update_line_button_guard.js',
        ],
    
    },
    'installable': True,
    'application': True,
}